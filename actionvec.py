...
action_filled_index = tf.placeholder(tf.int32, shape=([num_processing_steps, batch_size_tr, dimension]))
...
training_input, training_output = datas.getTrainData(sta_vecs, goal_graph, action_filled, num_processing_steps,
                                                     batch_size_tr)
...
goal_graph_tuple_test = utils_tf.placeholders_from_data_dicts(
    [base_graph(np.ones(grounding_size), grounding_size, first_action, dimension, nodes)])
...
progress_onestep_graph, round_progress_onestep_graph = model.processOneStep(output_ops_tests,
                                                                            action_filled_plan_onestep)
...
with tf.Session() as sess:
    ...
    for iteration in range(num_training_iterations):
        ...
        for j in range(batch_size_tr):
            ...
            input_goal_g = base_graph(state[j][num_processing_steps], grounding_size, first_action, dimension,
                                      proposition_nodes)
    ...
    for iteration in range(test_prob_size):
        ...
        goal_graph_test_g = base_graph(state_test[iteration][num_processing_steps], grounding_size, first_action,
        ...
        while num_step < num_processing_steps:
            ...
            while loop_index < len(action_dict) - 1:
                temp_act_sto = action_storage-test_values['act_vec']
                temp_res = np.sum(np.abs(temp_act_sto), axis=-1)

                maxIndex = np.argmax(temp_res)
                break
