...
def getComplementState(state_array, grounding):
    res = []
    for num_step_state in state_array:
        per_state = []
        for index in range(len(num_step_state)):
            if np.round(num_step_state[index][0]) != 0:
                per_state.append(grounding[index])
        res.append(per_state)
    return res
...
with tf.Session() as sess:
    ...
    proposition_nodes = (np.random.uniform(-6 / np.sqrt(dimension), 6 / np.sqrt(dimension), (grounding_size+1) * dimension))
    ...
    for iteration in range(num_training_iterations):
        ...
        if iteration % 5000 == 0:
            saver.save(sess, "./model/GNN-model-" + domain_name + "-n" + data_number + "-p" + save_per + ".ckpt")
    ...
    for iteration in range(dataset):
        graph_test_dicts = []
        input_g = base_graph(state_all[iteration][0], grounding_size, first_action, dimension,proposition_nodes)
        ...
        for num_step in range(num_processing_steps):
            act_fills.append([action_storage[acts_all[iteration][num_step]]])
        act_fills = np.array(act_fills)
        ...
        res = getComplementState(test_values["outputs"], grounding)
    ...
    saver.save(sess, "./model/GNN-model-" + domain_name + "-n" + data_number + "-p" + save_per+"-d" + str(dimension) + ".ckpt")
    with open(domain_name + "-n" + data_number + "-p" + save_per +"-d" + str(dimension)+ "_action.csv", 'w', newline='') as f:
        writer = csv.writer(f)
        for arr_ele in action_storage:
            writer.writerow(arr_ele)
    with open(domain_name + "-n" + data_number + "-p" + save_per +"-d" + str(dimension)+ "_pnode.csv", 'w', newline='') as f:
        writer = csv.writer(f)
