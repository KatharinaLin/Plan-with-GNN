...
class GraphProcess(snt.AbstractModule):
  ...
  def _build(self, input_op, action_sequence, num_processing_steps, flags):
    ...
    prev_output_trans = output_trans
    prev_latent = []
    for i in range(num_processing_steps):
      ...
      output_trans = tf.cond(tf.equal(action_sequence[i], self._action_set_size - 1), lambda: prev_output_trans,
                             lambda: output_trans)
      if i == 0:
        prev_latent = latent
      latent = tf.cond(tf.equal(action_sequence[i], self._action_set_size - 1), lambda: prev_latent, lambda: latent)
      prev_latent = latent
  ...
  def getInitialVecForState(self, graph):
    latent = self.stateNodeAdjustment(graph)
    action_index = [0]
  ...
  def getOriginActionStore(self):
    return self._action_store_node

  def getPNode(self):
    return self.p_node_store

  def setPNode(self, restore_p_node):
    self.p_node_store = tf.convert_to_tensor(restore_p_node
                                             )

  def setActionStore(self, read_in_data):
    self.output_action = tf.convert_to_tensor(read_in_data)
    self._action_store_node = self.output_action
