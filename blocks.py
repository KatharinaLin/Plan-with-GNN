...
class NodeBlock(snt.AbstractModule):
  ...
  def _build(self, graph):
    ...
    """
    split_egdes = [tf.split(graph.edges,graph.n_edge, 0)][0]
    split_nodes = [tf.split(graph.nodes,graph.n_node, 0)][0]
    updated_nodes_array = []
    for split_edge, split_node in zip(split_egdes, split_nodes):

      nodes_to_collect = []

      if self._use_received_edges:
        nodes_to_collect.append(split_edge)
    ...
    #   nodes_to_collect.append(self._sent_edges_aggregator(graph))
      ...
      if self._use_nodes:
        ...
        nodes_to_collect.append(split_node[1:])



      updated_nodes = tf.stack([tf.reduce_sum(nodes_to_collect[0]*nodes_to_collect[1],axis=0)])
      updated_array = []
      updated_array.append(updated_nodes)
      updated_array.append(split_node[1:])
      updated_array = tf.concat(updated_array, axis=0)
      updated_nodes_array.append(updated_array)
    updated_nodes_array = tf.concat(updated_nodes_array, axis=0)
    return graph.replace(nodes=updated_nodes_array)
