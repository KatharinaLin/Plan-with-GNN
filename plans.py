...
with tf.Session() as sess:
        ...
        if (iteration*batch_size_tr) % dataset > 0:
            ...
            for num_step in range(num_processing_steps):
                for batch_index in range(batch_size_tr):
                    # if acts[batch_index][num_step] == 30:
                    #     if num_step == 0:
                    #         print(state[batch_index][0])
                    #     else:
                    #         print()
                    if num_step == 0:
                        ...
                        negative_effect[acts[batch_index][num_step]] = negative_effect[acts[batch_index][num_step]] + \
                                                                       complete_states[num_step - 1][batch_index]
        sr = train_values["outputs"]
        sr = np.array(sr)

        for num_step in range(0,len(sr)):
            gg = 0
            batch_flag = 0
            while batch_flag < batch_size_tr:
                gg = 0
                act_name = acts[batch_flag][num_step]
                if num_step == 0:
                    gg = 0
                    while gg < len(grounding):
                        init_s = state[batch_flag][num_step][gg]
                        if init_s < 0:
                            init_s = 0
                        if np.round(sr[num_step][gg + batch_flag * len(grounding)][0]) < init_s:
                            if act_name in del_effect:
                                del_effect[act_name] += ("&"+grounding[gg])
                            else:
                                del_effect[act_name] = "&" +grounding[gg]
                        if np.round(sr[num_step][gg + batch_flag * len(grounding)][0]) > init_s:
                            if act_name in add_effect:
                                add_effect[act_name] += ("&" + grounding[gg])
                            else:
                                add_effect[act_name] = "&" +grounding[gg]
                        gg += 1
                else:
                    while gg < len(grounding):
                        if np.round(sr[num_step][gg + batch_flag * len(grounding)][0]) < np.round(
                                sr[num_step - 1][gg + batch_flag * len(grounding)][0]):
                            if act_name in del_effect:
                                del_effect[act_name] += ("&" + grounding[gg])
                            else:
                                del_effect[act_name] = "&" +grounding[gg]
                        ...
                        gg += 1
                if act_name in add_effect:
                    add_effect[act_name] += "_"
                else:
                    add_effect[act_name] = "_"
                if act_name in del_effect:
                    del_effect[act_name] += "_"
                else:
                    del_effect[act_name] = "_"
                batch_flag += 1
    ...
    for key,values in  action_dict.items():
        key_buffer = key.split(' ')
        action_name = ""
        for kkey in key_buffer:
            if action_name == "":
                action_name += kkey
            else:
                action_name += ("-"+kkey)
        print("(:action ", action_name)
        precondition_out = ":precondition (and"
        for pindex in range(len(positive_effect[values])):
            if positive_effect[values][pindex] == 1:
                precondition_out += (" ("+grounding[pindex] + ")")
        precondition_out += ")\n"
        print(precondition_out)


        effect_out = ":effect (and"
        if values in add_effect:
            addlist = add_effect[values].split("_")
            addset = set(addlist)
            add_max = ''
            maxCount = 0
            for item in addset:
                if addlist.count(item) > maxCount:
                    maxCount = addlist.count(item)
                    add_max = item
            add_max = add_max.split('&')[1:]
            for m in add_max:
                effect_out += (" ("+m + ")")

        if values in del_effect:
            dellist = del_effect[values].split("_")
            delset = set(dellist)
            del_max = ''
            maxCount = 0
            for item in delset:
                if dellist.count(item) > maxCount:
                    maxCount = dellist.count(item)
                    del_max = item
            del_max = del_max.split('&')[1:]
    ...
    savers.save(sess,"./model/GNN-model-"+typea+domain_name+"-n"+data_number+"-p"+save_per+".ckpt")
