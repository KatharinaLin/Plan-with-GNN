...
def error_calculation(output_ops_test, atom_num, states):
    ...
    return sum(error_array) / len(states)
...
with tf.Session() as sess:
    ...
    for iteration in range(last_iteration, num_training_iterations):
        ...
        train_values = sess.run({
            "step": step_op,
            "loss": loss_op_tr,
            "outputs": all_output_ops_tr,
            "test_ops": output_ops_test,
        }, feed_dict={graph_index: g_index, final:state_batch, action_seqs:act_indexs, graph_index_test: g_index_test, final_test: state_batch_test, action_seqs_test:act_indexs_test, hiddens: hidden_batch})
        the_time = time.time()
    ...
    saver.save(sess, "./model/GNN-model-"+domain_name+"-n"+data_number+"-p"+save_per+".ckpt")
    with open(domain_name+"-n"+data_number+"-p"+save_per+"_action.csv", 'w', newline='') as f:
        writer = csv.writer(f)
        for arr_ele in train_values['out']:
            writer.writerow(arr_ele)
    with open(domain_name+"-n"+data_number+"-p"+save_per+"_pnode.csv", 'w', newline='') as f:
        writer = csv.writer(f)
        for arr_ele in train_values['p_node']:
            writer.writerow(arr_ele)
